...
import dill
torch.utils.data.get_worker_info().data_loader._AutoCollationBatchReducer = dill.reducer
...
class Muscle(Dataset):
  def __init__(self, train = True, transformX = None, transformY = None):
    ...
    self.train = train
    self.complex_object = dill.dumps(your_complex_object)
  ...
  def __getitem__(self, index):
    ...
    complex_object = dill.loads(self.complex_object)
    if self.train:
